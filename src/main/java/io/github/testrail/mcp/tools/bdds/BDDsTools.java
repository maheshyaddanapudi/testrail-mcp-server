...
public class BDDsTools {
    ...
    @InternalTool(
            ...
            category = "bdd",
            examples = {
                    "execute_tool('get_bdd', {caseId: 2133})",
                    "execute_tool('get_bdd', {caseId: 100})"
            },
            keywords = {"get", "export", "retrieve", "fetch", "download", "bdd", "scenario", "feature", "gherkin", "cucumber"}
    )
    public String getBdd(
            @InternalToolParam(description = "The ID of the test case to export as BDD scenario")
            int caseId
    ) {
    ...
    @InternalTool(
            ...
            category = "bdd",
            examples = {
                    "execute_tool('add_bdd', {sectionId: 188, featureFileContent: 'Feature: Login\\n  Scenario: Valid login\\n    Given user is on login page\\n    When user enters credentials\\n    Then user is logged in'})",
                    "execute_tool('add_bdd', {sectionId: 50, featureFileContent: 'Feature: Search\\n  Scenario: Search products\\n    Given user is on home page\\n    When user searches for product\\n    Then results are displayed'})"
            },
            keywords = {"add", "import", "upload", "create", "bdd", "scenario", "feature", "gherkin", "cucumber"}
    )
    public TestCase addBdd(
            @InternalToolParam(description = "The ID of the section to add the BDD scenario to")
            int sectionId,
            @InternalToolParam(description = "Feature file content in Gherkin syntax (Feature, Scenario, Given/When/Then)")
