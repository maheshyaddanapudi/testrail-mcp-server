...
public class PlansTools {
    ...
    @InternalTool(
            ...
            category = "test-plans",
            examples = {
                    "execute_tool('get_plan', {planId: 123})",
                    "execute_tool('get_plan', {planId: 456})"
            },
            keywords = {"get", "retrieve", "fetch", "show", "view", "plan", "status", "progress", "statistics"}
    )
    public TestPlan getPlan(
            @InternalToolParam(description = "The unique identifier of the test plan.")
            Integer planId
    ) {
    ...
    @InternalTool(
            ...
            category = "test-plans",
            examples = {
                    "execute_tool('get_plans', {projectId: 1})",
                    "execute_tool('get_plans', {projectId: 5, isCompleted: 0})",
                    "execute_tool('get_plans', {projectId: 3, isCompleted: 1, limit: 50})",
                    "execute_tool('get_plans', {projectId: 2, milestoneId: '5'})"
            },
            keywords = {"get", "list", "retrieve", "fetch", "show", "browse", "plans", "all", "history", "active", "completed"}
    )
    public List<TestPlan> getPlans(
            @InternalToolParam(description = "The ID of the project to retrieve test plans from.")
            Integer projectId,
            @InternalToolParam(description = "Only return plans created after this UNIX timestamp", required = false)
            Long createdAfter,
            @InternalToolParam(description = "Only return plans created before this UNIX timestamp", required = false)
            Long createdBefore,
            @InternalToolParam(description = "Comma-separated list of creator user IDs to filter by", required = false)
            String createdBy,
            @InternalToolParam(description = "Filter by completion status: 1 for completed, 0 for active, null for all", required = false)
            Integer isCompleted,
    ...
    @InternalTool(
            ...
            category = "test-plans",
            examples = {
                    "execute_tool('add_plan', {projectId: 1, name: 'Sprint 23 Testing'})",
                    "execute_tool('add_plan', {projectId: 2, name: 'Browser Testing', milestoneId: 5})",
                    "execute_tool('add_plan', {projectId: 3, name: 'Release 2.0', description: 'Final release testing', startOn: 1640000000, dueOn: 1650000000})"
            },
            keywords = {"add", "create", "new", "start", "plan", "test", "organize", "group"}
    )
    public TestPlan addPlan(
            @InternalToolParam(description = "The ID of the project to add the test plan to.")
            Integer projectId,
            @InternalToolParam(description = "The name of the test plan.")
            String name,
            @InternalToolParam(description = "Optional description of the test plan", required = false)
            String description,
            @InternalToolParam(description = "Optional milestone ID to link to the plan", required = false)
            Integer milestoneId,
            @InternalToolParam(description = "Optional start date as UNIX timestamp", required = false)
            Long startOn,
            @InternalToolParam(description = "Optional due date as UNIX timestamp", required = false)
            Long dueOn
    ) {
    ...
    @InternalTool(
            ...
            category = "test-plans",
            examples = {
                    "execute_tool('update_plan', {planId: 123, name: 'Sprint 24 Testing'})",
                    "execute_tool('update_plan', {planId: 456, milestoneId: 5})",
                    "execute_tool('update_plan', {planId: 789, dueOn: 1640000000})"
            },
            keywords = {"update", "modify", "change", "edit", "revise", "plan", "settings"}
    )
    public TestPlan updatePlan(
            @InternalToolParam(description = "The ID of the test plan to update.")
            Integer planId,
            @InternalToolParam(description = "Optional new name for the plan", required = false)
            String name,
            @InternalToolParam(description = "Optional new description", required = false)
            String description,
            @InternalToolParam(description = "Optional new milestone ID", required = false)
            Integer milestoneId,
            @InternalToolParam(description = "Optional new start date as UNIX timestamp", required = false)
            Long startOn,
            @InternalToolParam(description = "Optional new due date as UNIX timestamp", required = false)
            Long dueOn
    ) {
    ...
    @InternalTool(
            ...
            category = "test-plans",
            examples = {
                    "execute_tool('close_plan', {planId: 123})",
                    "execute_tool('close_plan', {planId: 456})"
            },
            keywords = {"close", "finalize", "complete", "archive", "lock", "plan", "finish"}
    )
    public TestPlan closePlan(
            @InternalToolParam(description = "The ID of the test plan to close.")
            Integer planId
    ) {
    ...
    @InternalTool(
            ...
            category = "test-plans",
            examples = {
                    "execute_tool('delete_plan', {planId: 123})",
                    "execute_tool('delete_plan', {planId: 456})"
            },
            keywords = {"delete", "remove", "erase", "destroy", "purge", "plan", "cleanup"}
    )
    public OperationResult deletePlan(
            @InternalToolParam(description = "The ID of the test plan to delete.")
            Integer planId
    ) {
